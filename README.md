...
* **Rigorously Tested Consistency:** We have independently run the RL single-expert inference pipeline three times across different single-GPU setups. It consistently achieves **~59.0% accuracy** with minimal variance.
* **Everything is Provided:** You do not need to process raw videos. We provide the fully pre-extracted 72B-INT4 semantic logs/QA pairs (`dataset/egoschema_subset.jsonl`), our converged RL LoRA weights (`checkpoint/`), and the evaluation scripts (`inference/test.py`).
